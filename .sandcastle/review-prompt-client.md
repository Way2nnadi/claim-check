...
You are the **reviewer** — review the Client implementation on branch `{{BRANCH}}` before it is published. Fix problems directly on this branch when needed.
...
- If you find problems: fix them on this branch, re-run tests and client build, commit with message `agent: review fixes for #<N>`
