...
import { formatExtractionRunStatus, formatPinningLabel } from "./extractionRunFormat";
        ...
        return (
          <li key={run.extraction_run_id}>
            <article
              ...
              style={{ "--reveal-delay": `${40 + index * 45}ms` } as CSSProperties}
            >
              <header className="extraction-run-head">
                <div className="extraction-run-idline">
                  <code>{run.extraction_run_id}</code>
                  <span
                ...
                <time className="extraction-run-time" dateTime={run.created_at}>
              ...
              {showDocumentContext ? (
                <dl className="extraction-context-grid">
                  <div>
                    <dt>Document</dt>
                    <dd>{run.document_id}</dd>
                  </div>
                  <div>
                    <dt>Version</dt>
                    <dd>{run.document_version_id}</dd>
                  </div>
                </dl>
              ) : null}
              ...
              <dl className="extraction-pin-grid">
                <div>
                  ...
                  <dd>
                    {formatPinningLabel(run.prompt_template_id, run.prompt_template_version)}
                  </dd>
                </div>
                <div>
              ...
              {isFailed && run.failure_detail ? (
                <div className="extraction-failure" role="alert">
                  <span className="extraction-failure-label">Failure detail</span>
